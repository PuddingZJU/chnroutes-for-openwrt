...
def generate_openwrt(metric,oldgateway,olddev):
    ...
    for ip,mask,_ in results:
        upfile.write('route add -net %s netmask %s gateway %s dev %s\n'%(ip,mask,oldgateway,olddev))
        downfile.write('route del -net %s netmask %s\n'%(ip,mask))
...
if __name__=='__main__':
    ...
    elif args.platform.lower() == 'openwrt':
        generate_openwrt(args.metric,args.oldgateway,args.olddev)
